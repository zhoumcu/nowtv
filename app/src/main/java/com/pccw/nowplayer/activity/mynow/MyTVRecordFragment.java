 ...
 */
public class MyTVRecordFragment extends BaseFragment {
    ...
    private void initViews() {
        ...
        emptyText.setText(getString(R.string.no_tv_record_item));
    ...
    private void bindViews() {
        ...
        PVRClient.getInstance().loadPVRList().always(new AlwaysCallback<List<Node>, Throwable>() {
            @Override
            public void onAlways(Promise.State state, List<Node> resolved, Throwable rejected) {
                if (!isAdded()) return;
                progressLay.setVisibility(View.GONE);
                subNodes.clear();
                if (resolved != null) subNodes.addAll(resolved);
                refreshViews();
            }
        });
    }
